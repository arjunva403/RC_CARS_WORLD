...
const offerPageLoad = async (req, res) => {
  try {
    ...
    // Sync product offers
    await model.offerModle.updateMany(
    ...
    await model.categoryOffer.updateMany(
      { endDate: { $lt: now }, status: { $ne: "inactive" } },
      { $set: { status: "inactive" } }
    );
    await model.categoryOffer.updateMany(
    ...
    // Sync brand offers
    await model.brandOffer.updateMany(
      { endDate: { $lt: now }, status: { $ne: "inactive" } },
      { $set: { status: "inactive" } }
    );
    await model.brandOffer.updateMany(
    ...
    const categoriesOffers = await model.categoryOffer
      .find()
      .populate("categoryId");
...
const offerPost = async (req, res) => {
  try {
    ...
    if (!title) return res.json({ fail: "title not entered" });
    if (!productID) return res.json({ fail: "product not seleted" });
    if (!discount) return res.json({ fail: "discount not enterd" });
    if (!startDate) return res.json({ fail: "enter your Offer Starting Date" });
    if (!endDate) return res.json({ fail: "enter your Offer Ending Date" });
    ...
    const now = new Date();
    const status =
      end < now ? "inactive" : start <= now ? "active" : "inactive";

    const newOffer = new model.offerModle({
      title,
      productId: productID,
      discount: disc,
      status,
...
const updateOffer = async (req, res) => {
  try {
    ...
    if (!offerId) return res.json({ fail: "id is not there" });
    if (!title) return res.json({ fail: "title is not entered" });
    if (!productID) return res.json({ fail: "productID is not entered" });
    if (!discount) return res.json({ fail: "discount is not entered" });
    if (!startDate) return res.json({ fail: "startDate is not entered" });
    if (!endDate) return res.json({ fail: "endDate is not entered" });
    ...
    res
      .status(200)
      .json({ success: "Offer updated successfully", offer: updatedOffer });
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Server Error");
  }
